...
for k=1:length(names)
    ...
    acc_raw_h = [acc_raw; 1]; %we add a 1 at the end of the vector (homogeneous coordinates)
    % Transform acc_raw to a_0
    a_0_h = T_acc_0 * acc_raw_h; %multiply the transformation matrix by the homogeneous vector
    a_0 = a_0_h(1:3); %back to 3d
    %??????????????????????????????????????????????????????????????????????
