...
acc_h = [acc_raw; ones(1, size(acc_raw,2))]; % convert acc_raw into 4x1
acc_cal_h = T * acc_h; % apply transformation matrix
acc_0 = acc_cal_h(1:3, :); % back into 3x1
...
while 1
    ...
    for k = 1:length(scswp)
        ...
        for l=1:length(ids)
            ...
            acc_h = [acc_raw; ones(1, size(acc_raw,2))]; % 4x1
            acc_cal_h = T * acc_h; % apply transformation
            acc_n = acc_cal_h(1:3, :); % back to 3xP
            ...
            % estRot gives rotation matrix that maps from a to b; 
            % so if we here want rotation root to neighbour (because that
            % is what rotation of neighbour with respect to root means),
            % then need rotation matrix 0 to n
            R_n_0 = estRot(acc_0, acc_n); % get R from 0 to n 
            ...
            % ports on root and neighbour must be in same position because
            % they are the connection point in the real world --> 
            % p0 + R0*pP0 = pn + Rn*pPn
            pn = p0 + R0*pP0 - Rn*pPn;
